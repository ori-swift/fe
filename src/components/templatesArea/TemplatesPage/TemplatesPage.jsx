...
const TemplatesPage = () => {
  ...
  const handleEdit = (template) => { setEditingTemplate(template); setShowModal(true); };
  const handleDelete = (id) => console.log('מחיקת תבנית עם מזהה:', id);
  const handleCreate = () => { setEditingTemplate(null); setShowModal(true); };
  const handleCloseModal = () => { setShowModal(false); setEditingTemplate(null); };
      ...
      <div className="templates-section">
        ...
        <div className="templates-table-wrapper">
          <Table striped bordered hover responsive className="templates-table">
            <thead><tr><th>מזהה</th><th>אמצעי</th><th>סוג</th><th>לקוח</th><th>מסמך</th><th>שלב</th><th>מרוכז</th><th>תוכן</th><th>פעולות</th></tr></thead>
            <tbody>{filteredUniqueTemplates.length ? filteredUniqueTemplates.map(([_, t]) => (
              <tr key={t.id}><td>{t.id}</td><td><Badge bg={t.alert_method === 'email' ? 'primary' : t.alert_method === 'sms' ? 'success' : 'info'}>{t.alert_method}</Badge></td><td>{t.doc_type || 'ללא'}</td><td>{t.client_id || 'ללא'}</td><td>{t.document_id || 'ללא'}</td><td>{t.phase_number || 'ללא'}</td><td>{t.is_aggregate ? 'כן' : 'לא'}</td><td>{t.template_content.substring(0, 80)}{t.template_content.length > 80 && '...'}</td><td><Button variant="outline-primary" size="sm" className="me-1" onClick={() => handleEdit(t)}><FaEdit /></Button><Button variant="outline-danger" size="sm" onClick={() => handleDelete(t.id)}><FaTrash /></Button></td></tr>
